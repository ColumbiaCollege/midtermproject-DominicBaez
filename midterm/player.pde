...
//methods
void move(){
  // if the 'a' key is pressed then the player will move at pixel along the x-axis
  if (left) {
      xPos = xPos-1;
    ...
    //if the 'd' key is pressed then the player will move at a pixel along the x-axis
    if (right) {
      xPos = xPos+1;
      //wrapping condition where the player will return to the other side of the screen when it goes offscreen
      if (xPos>width) {
